...
flights <- mutate(flights,
  ...
  fly_time_min = case_when(
            (arr_time - dep_time) < 0 ~ (arr_time_min - dep_time_min + 1440),
            (arr_time > dep_time) ~  (arr_time_min - dep_time_min))
) 
select(flights, dep_time, dep_time_min, arr_time, arr_time_min, air_time, fly_time_min) #exercise 2, last two col compare air time to fly time
```
...
```{r 5.5, practice}
head(arrange(flights, min_rank(desc(flights$dep_delay))), 10)
#exercise 4
```
